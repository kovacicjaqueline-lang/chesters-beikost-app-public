...
- innerhalb eines Runner-/Shard-Laufs werden standardmäßig worker-lokale gemeinsame WebKit-Browserprozesse über Playwright `launchServer()`/`connect()` wiederverwendet; jeder Test behält seinen eigenen HTTP-Server und Browser-Context. Explizite `webkit.launch(...)`-Optionen fallen auf einen dedizierten Browserstart zurück,
