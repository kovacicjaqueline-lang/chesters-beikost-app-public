...
function measureVisibleGeometry(decoded, alphaThreshold = ALPHA_GEOMETRY_THRESHOLD) {
  ...
  assert.ok(maxX >= minX && maxY >= minY, `keine sichtbaren Pixel ab Alpha ${alphaThreshold}`);
  ...
  const canvasCenterY = decoded.height / 2;
  const margins = {
    left: minX,
    top: minY,
  ...
  return {
    ...
    centerX,
    centerY,
    centerOffsetX: centerX - canvasCenterX,
    centerOffsetY: centerY - canvasCenterY,
    margins,
