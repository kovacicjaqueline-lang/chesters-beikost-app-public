...
function assertV2Asset(root, relativePath) {
  ...
  assert.equal(opening.match(/\bwidth=["']([^"']+)["']/i)?.[1], "128", `${relativePath}: SVG width muss 128 sein`);
  assert.equal(opening.match(/\bheight=["']([^"']+)["']/i)?.[1], "128", `${relativePath}: SVG height muss 128 sein`);
  assert.equal(opening.match(/\bviewBox=["']([^"']+)["']/i)?.[1], "0 0 128 128", `${relativePath}: SVG viewBox`);

  const decoded = decodeEmbeddedPng(embeddedPngFromSvg(svg, relativePath), relativePath);
  for (let x = 0; x < decoded.width; x++) {
    assert.equal(decoded.alphaAt(x, 0), 0, `${relativePath}: oberer Außenrand muss transparent sein`);
    assert.equal(decoded.alphaAt(x, decoded.height - 1), 0, `${relativePath}: unterer Außenrand muss transparent sein`);
  }
  for (let y = 0; y < decoded.height; y++) {
    assert.equal(decoded.alphaAt(0, y), 0, `${relativePath}: linker Außenrand muss transparent sein`);
    assert.equal(decoded.alphaAt(decoded.width - 1, y), 0, `${relativePath}: rechter Außenrand muss transparent sein`);
  }
